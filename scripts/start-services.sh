...
build() {
  docker build -f "$home"/services/liquibase/main-db/Dockerfile . --no-cache -t wrc-liquibase-db-main:latest
...
#  docker compose -f "$home"/infrastructure/compose.dev.db.yaml down --remove-orphans
...
#  docker compose -f "$home"/infrastructure/compose.dev.services.yaml down --remove-orphans
